 ...
 */
//自定义MyDiscovery的原因在于原DiscoveryTest中test1和test2方法用同一个sourcePort，造成若初始化时指定了sourcePort，则运行test方法检测nat类型时报错
public class MyDiscovery {
    ...
    public MyDiscovery(InetAddress sourceIaddress, int sourcePort, String stunServer, int stunServerPort) {
    ...
    public MyDiscoveryInfo test() throws UtilityException, SocketException, UnknownHostException, IOException, MessageAttributeParsingException, MessageAttributeException, MessageHeaderParsingException {
        ma = null;
        ca = null;
        nodeNatted = true;
        socketTest1 = null;
        di = new MyDiscoveryInfo(sourceIaddress);
    ...
    private boolean test2() throws UtilityException, SocketException, UnknownHostException, IOException, MessageAttributeParsingException, MessageAttributeException, MessageHeaderParsingException {
        ...
        while (true) {
            try {
                ...
                DatagramSocket sendSocket = new DatagramSocket(new InetSocketAddress(sourceIaddress, 0));
